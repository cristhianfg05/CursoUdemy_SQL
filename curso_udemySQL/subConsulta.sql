...
drop database if exists subconsultas;
...
select titulo from libros where precio = (select max(precio)from libros)) as "libro caro" from libros where titulo='uno';
...
drop table if exists libros;
 ...
 insert into libros(titulo,autor,codigoeditorial,precio) values('Cervantes y el quijote','Borges',3,25);
  ...
  select titulo from libros where autor='Borges' and codigoeditorial = some 
